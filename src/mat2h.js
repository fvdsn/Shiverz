...
(function(modula){
	...
	proto.equals = function(mat){
		return  this.full_type === mat.full_type && 
			...
			epsilon_equals(this.xz, mat.xz) &&
			epsilon_equals(this.yz, mat.yz) &&
	...
	Mat2h.mult_vec = function(m,vec,dst){
		// xx xy xz   x 
		// yx yy yz * y 
		// 0  0  1    1 
	...
	Mat2h.set_rotation = function(m,angle){
	...
	Mat2h.rotation = function(angle){
		var m = new Mat2h();
		Mat2h.set_rotation(m,angle);
		return m;
	};

	Mat2h.rotation_deg = function(angle){
		return Mat2h.rotation(angle * modula.deg2rad);
	};

	Mat2h.set_scale = function(m,scale){
		m.xx = scale.x;
		m.xy = 0;
		m.yx = 0;
		m.yy = scale.y;
	...
	Mat2h.scale = function(scale){
		var m = new Mat2h();
		Mat2h.set_scale(m,scale);
		return m;
	};

	Mat2h.set_translation = function(m,vec){
		m.xx = 1;
		m.xy = 0;
		m.yx = 0;
		m.yy = 1;;
